...
`sx-dispatch` 是 Skillix 的智能分流工具，负责分析用户任务并推荐最佳操作。
...
**核心思想**：
- dispatch 工具只负责**收集信息**（本地技能、市场技能）
- **决策权交给 AI**
- AI 根据任务描述和技能列表自行判断最佳匹配

```mermaid
flowchart TD
    A[用户任务] --> B[sx-dispatch]
    B --> C[收集技能信息]
    C --> D[返回结构化数据]
    D --> E[AI 自行分析决策]
    E --> F[执行推荐操作]
