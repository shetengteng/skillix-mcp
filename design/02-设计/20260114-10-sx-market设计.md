...
## 十七、与 sx-dispatch 的协作

### 17.1 协作弥补

sx-dispatch 智能分流机制可以弥补部分不足：

| 局限 | sx-dispatch 弥补情况 |
|------|-------------------|
| 搜索功能较弱 | ✅ **已弥补** - sx-dispatch 提供智能匹配算法（TF-IDF、余弦相似度） |
| 缺少技能预览 | ✅ **已弥补** - 分流后调用 `sx-skill read` 实现预览 |
| 离线场景 | ⚠️ **部分弥补** - sx-dispatch 有降级策略，网络错误时仅使用本地技能 |
| 技能依赖关系 | ⚠️ **部分弥补** - sx-dispatch 有 `COMPOSE` 操作支持组合技能 |

### 17.2 推荐使用方式

```mermaid
flowchart LR
    A[用户请求] --> B[sx-dispatch<br/>智能分流]
    B --> C{决策}
...
**建议**：
- 用户通过 sx-dispatch 作为入口，由其自动判断是否需要调用 sx-market
- sx-dispatch 的智能匹配弥补了 sx-market 简单搜索的不足
- 降级策略提供了一定的容错能力
