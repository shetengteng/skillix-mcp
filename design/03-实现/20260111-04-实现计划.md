...
- [ ] sx-dispatch 工具（智能分流）
- [ ] 自动检测重复模式
- [ ] 自动建议功能（集成在 sx-dispatch 中）

> **注意**：原设计中的 `sx_suggest`、`sx_feedback`、`sx_analyze` 已整合到 `sx-dispatch` 工具和配置系统中，不再作为独立工具实现。
...
我们在 `sx-dispatch` 工具中实现了类似的机制，并新增了 `INSTALL` 动作。
