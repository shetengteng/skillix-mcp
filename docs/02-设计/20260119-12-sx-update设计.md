...
> ⚠️ **重要说明**：当前代码实现中**没有**内置的版本回退机制。

**现有代码特点**：
- `updateSkill` 函数直接覆盖 SKILL.md 文件
- 不保存历史版本
- 不记录更新日志
- 没有自动备份机制
...
| 功能 | 优先级 | 状态 | 说明 |
|------|--------|------|------|
| evolution.log 记录 | 中 | 待实现 | 记录每次更新的变更日志 |
| .versions/ 备份目录 | 中 | 待实现 | 本地创建技能的版本历史 |
| sx-skill rollback | 中 | 规划中 | 回退到指定版本 |
| sx-skill history | 低 | 规划中 | 查看版本历史 |
...
export function updateSkill(
  ...
  const skill = getSkill(skillName, options?.projectRoot);
  ...
  // 3. 记录到 evolution.log
  logEvolution(skill, updates, options?.reason);
  
  return updatedSkill;
}
```

#### 11.8.2 rollback 函数设计
...
export function rollbackSkill(
  ...
  // 检查技能来源
  const installedInfo = getInstalledInfo(skillName);
  
  if (installedInfo) {
    // Market 安装的技能：从源重新安装
    return reinstallFromSource(skillName, targetVersion, installedInfo.sourceId);
  } else {
    // 本地创建的技能：从 .versions 恢复
    return restoreFromBackup(skill, targetVersion);
  }
}
```
