...
- `tests/utils/markdown.test.ts` - Markdown 解析测试
- `tests/services/config.test.ts` - 配置服务测试
- `tests/services/skill.test.ts` - 技能服务测试
- `tests/services/triage.test.ts` - 分流服务测试
- `tests/services/version.test.ts` - 版本管理服务测试 ✨ 新增
- `tests/services/market.test.ts` - 市场服务测试 ✨ 新增
- `tests/tools/skills.test.ts` - sx-skill 工具测试
- `tests/tools/configs.test.ts` - sx-config 工具测试
- `tests/tools/helps.test.ts` - sx-help 工具测试
- `tests/tools/triages.test.ts` - sx-triage 工具测试
...
describe('skill service', () => {
  ...
  describe('updateSkill', () => {
    ✅ 'should update existing skill metadata'
    ✅ 'should update skill body only'
    ✅ 'should return null for non-existing skill'
    ✅ 'should preserve unchanged metadata fields when updating partially'
    ✅ 'should update both metadata and body simultaneously'
    ✅ 'should preserve original body when only updating metadata'
...
#### version.test.ts ✅ (14 个测试) - 新增

```typescript
describe('version service', () => {
  describe('createVersionBackup', () => {
    ✅ 'should create backup of existing skill'
    ✅ 'should create multiple backups with incrementing numbers'
  });

  describe('listVersions', () => {
    ✅ 'should list all backup versions'
    ✅ 'should return empty array when no backups exist'
  });

  describe('updateSkill with backup', () => {
    ✅ 'should create backup when updating skill (default behavior)'
    ✅ 'should not create backup when createBackup is false'
    ✅ 'should record reason in evolution log'
  });

  describe('rollbackSkill', () => {
    ✅ 'should rollback to previous version'
    ✅ 'should fail when target version does not exist'
    ✅ 'should fail when skill does not exist'
  });

  describe('rollbackToPrevious', () => {
    ✅ 'should rollback to the most recent backup'
  });
...
#### market.test.ts ✅ (18 个测试, 3 跳过) - 新增

```typescript
describe('market service', () => {
  describe('URL parsing', () => {
    ✅ 'should parse GitHub URL correctly'
    ✅ 'should generate correct source ID'
    ✅ 'should handle invalid URL'
  });

  describe('cache paths', () => {
    ✅ 'should return correct repos cache dir'
    ✅ 'should return correct indexes cache dir'
    ✅ 'should return correct manifest path'
  });

  describe('source ID conversion', () => {
    ✅ 'should convert source ID to dir name'
    ✅ 'should convert dir name to source ID'
  });

  describe('network tests', () => {
    ⏭️ 'should sync source from GitHub' (需要网络)
    ⏭️ 'should search skills after sync' (需要网络)
    ⏭️ 'should install skill from synced source' (需要网络)
  });

  describe('manifest operations', () => {
    ✅ 'should update and load manifest'
    ✅ 'should remove source from manifest'
  });

  describe('source index operations', () => {
    ✅ 'should save and load source index'
  });

  describe('installed record operations', () => {
    ✅ 'should return null when no installed record exists'
    ✅ 'should check if skill is installed'
  });

  describe('status operations', () => {
    ✅ 'should return status with summary'
...
describe('sx-skill tool', () => {
  ...
  describe('update action', () => {
    ✅ 'should update existing project skill metadata'
    ✅ 'should fail without name parameter'
    ✅ 'should fail without update content (no metadata or body)'
    ✅ 'should fail for non-existing skill'
    ✅ 'should update skill body only'
    ✅ 'should update both metadata and body'
    ✅ 'should return updated skill info in response data'
  });
...
|------|-----------|----------|
| `src/utils/` | 90%+ | ✅ 已测试 |
| `src/services/skill/` | 85%+ | ✅ 已测试 |
| `src/services/config/` | 85%+ | ✅ 已测试 |
| `src/services/triage/` | 85%+ | ✅ 已测试 |
| `src/services/market/` | 80%+ | ✅ 已测试 |
| `src/tools/` | 80%+ | ✅ 已测试 |
| `src/index.ts` | 70%+ | 待测试 |
| **总体** | **80%+** | 进行中 |
...
Market 服务的网络测试需要设置环境变量 `RUN_NETWORK_TESTS=true` 来运行：

```bash
# 运行包含网络测试的完整测试
RUN_NETWORK_TESTS=true npm test -- --run

# 仅运行 market 网络测试
RUN_NETWORK_TESTS=true npm test -- --run tests/services/market.test.ts
```

网络测试使用 https://github.com/anthropics/skills 作为测试源。
...
```
 ✓ tests/utils/paths.test.ts  (12 tests) 3ms
 ✓ tests/utils/markdown.test.ts  (13 tests) 4ms
 ✓ tests/utils/fs.test.ts  (25 tests) 125ms
 ✓ tests/services/config.test.ts  (13 tests) 195ms
 ✓ tests/services/market.test.ts  (18 tests | 3 skipped) 215ms
 ✓ tests/services/triage.test.ts  (8 tests) 136ms
 ✓ tests/services/skill.test.ts  (20 tests) 430ms
 ✓ tests/services/version.test.ts  (14 tests) 491ms
 ✓ tests/tools/configs.test.ts  (9 tests) 59ms
 ✓ tests/tools/triages.test.ts  (16 tests) 221ms
 ✓ tests/tools/helps.test.ts  (5 tests) 13ms
 ✓ tests/tools/skills.test.ts  (18 tests) 278ms
