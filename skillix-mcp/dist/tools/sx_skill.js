...
exports.sxSkillDefinition = void 0;
exports.sxSkill = sxSkill;
...
function handleList(params) {
    ...
    try {
        const result = index_js_1.skillService.listAllSkills(projectRoot);
...
function handleRead(params) {
    ...
    try {
        const content = index_js_1.skillService.readSkillContent(name, projectRoot);
        if (!content) {
            return {
                success: false,
                message: `技能 "${name}" 不存在`,
...
function handleCreate(params) {
    ...
    // 检查技能是否已存在
    if (index_js_1.skillService.skillExists(name, projectRoot)) {
        return {
            success: false,
            message: `技能 "${name}" 已存在`,
    ...
    try {
        const skill = index_js_1.skillService.createSkill(name, metadata, body, scope, projectRoot);
        return {
            success: true,
            message: `成功创建技能 "${name}"`,
            data: {
...
function handleUpdate(params) {
    ...
    try {
        const skill = index_js_1.skillService.updateSkill(name, { metadata, body }, projectRoot);
        if (!skill) {
            return {
                success: false,
                message: `技能 "${name}" 不存在`,
...
function handleDelete(params) {
    ...
    try {
        const success = index_js_1.skillService.deleteSkill(name, projectRoot);
        if (!success) {
            return {
                success: false,
...
//# sourceMappingURL=sx_skill.js.map